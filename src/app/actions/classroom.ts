...
import { Classroom, FetchClassroomsParams } from "@/types/classroom";
...
export async function updateClassroomAction(formData: Classroom, id: string) {
    try {
...
export async function getAllClassroomsAction(params: FetchClassroomsParams) {
    try {
        const headers = await getAuthHeaders();
        // Axios limpiará automáticamente las propiedades undefined
        const response = await axios.get(`${BACKEND_URL}/classrooms`, {
            params,
            headers: headers
        });
