...
import { EmployeeFormData, FetchEmployeesParams } from "@/types/employee";
...
export async function getAllEmployeesAction(params: FetchEmployeesParams) {
    try {
        const headers = await getAuthHeaders();
...
export async function deleteEmployeeAction(id: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        await axios.delete(`${BACKEND_URL}/employees/${id}`, { headers });
