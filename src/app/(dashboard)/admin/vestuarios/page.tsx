...
export default function VestuariosPage() {
  ...
  const filteredVestuarios = DEMO_VESTUARIOS.filter((ves) => {
    const matchesSearch =
      ves.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ves.ritmo.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesEstado =
      estadoFilter === "Todos" || ves.estado === estadoFilter;
    return matchesSearch && matchesEstado;
  });

  // Métricas de inventario text-analíticas
  const totalTrajes = DEMO_VESTUARIOS.reduce(
    (acc, curr) => acc + curr.totalUnidades,
    0,
  );
  const trajesAsignados = DEMO_VESTUARIOS.reduce(
    (acc, curr) => acc + curr.asignados,
    0,
  );
  const enTaller = DEMO_VESTUARIOS.filter(
    (v) => v.estado === "En Costura",
  ).length;
          ...
          {filteredVestuarios.length > 0 ? (
            filteredVestuarios.map((ves) => {
              ...
              const porcentajeUso = Math.round(
                (ves.asignados / ves.totalUnidades) * 100,
              );
                    ...
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-questrial text-gray-400">
                            {ves.id}
                          </span>
                          <span
